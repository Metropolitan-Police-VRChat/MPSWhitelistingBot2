...
class VRChatAccoutLink(commands.Cog):
    ...
    async def link(self, ctx, vrchat_name):
        ...
        the MPS when entering the MPS Station. To use the
        ...
        a part of your name, example: $link "your \\\"vrchat\\\" name".
        """
        ...
        if previous_vrchat_name:
            confirm = await my_menus.Confirm(
                f'You already have a VRChat account registered witch is `"{previous_vrchat_name}"`, do you want to replace that account?').prompt(
                ctx)
            if not confirm:
                await ctx.send(
        ...
        confirm = await my_menus.Confirm(
            f'Are you sure `"{vrchat_name}"` is your full VRChat name?\n**You will be held responsible of the actions of the VRChat user with this name.**').prompt(
            ctx)
        if confirm:
    ...
    async def unlink(self, ctx):
        ...
        if vrchat_name is None:
            await ctx.send("You do not have your VRChat name linked.")
            return

        confirm = await my_menus.Confirm(
            f'Your VRChat name is currently set to `"{vrchat_name}"`. Do you want to unlink that?').prompt(ctx)
        if confirm:
    ...
    @commands.command()
    @checks.is_staff_role()
    @checks.is_admin_bot_channel()
    async def list_vrc_names(self, ctx):
        """
        This command is used to get the VRChat names of the people that are MPS Officers.
        ...
        if len(output_text) < 2000:
            ...
            with StringIO(output_text) as error_file_sio:
                with BytesIO(error_file_sio.read().encode('utf8')) as error_file:
                    await ctx.send("The output is too big to fit in a discord message so it is insted in a file.",
                                   file=discord.File(error_file, filename="all_vrc_names.txt"))
